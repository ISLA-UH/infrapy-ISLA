...
from ..typing.infrapy_types import InfrasoundDetection
...
#    Blastwave Model    #
#########################
def kg_op(W: float, r: float, p_amb: float = 101.325, T_amb: float = 288.15, exp_type: str = "chemical") -> float:
    """
...
def kg_ppd(W: float, r: float, p_amb: float = 101.325, T_amb: float = 288.15, exp_type: str = "chemical") -> float:
    """
...
def blastwave(t: float, W: float, r: float, p_amb: float = 101.325, T_amb: float = 288.15,
              exp_type: str = "chemical", shaping_param: float = 0.0) -> float:
    """
        ...
        exp_type: string
            Type of explosion modeled, options are "chemical" or "nuclear"
        shaping_param: float
            Shaping parameter for waveform that controls high frequency trend
            (set to 0 for original Friedlander blastwave)
    ...
    if len(np.atleast_1d(result)) == 1:
        ...
        result[x >= 0.0] = p0 / norm * x[x >= 0.0]**shaping_param * (1.0 - x[x >= 0.0]
                                                                     / (1.0 + shaping_param)) * np.exp(-x[x >= 0.0])
        result[x < 0.0] = 0.0
        return result


def blastwave_spectrum(f: float, W: float, r: float, p_amb: float = 101.325, T_amb: float = 288.15,
                       exp_type: str = "chemical", shaping_param: float = 0.0) -> float:
    """
    ...
    return p0 / norm * t0 * gamma(shaping_param + 1.0) * (omega * t0) \
        / (1.0 + (omega**2 * t0**2))**(shaping_param / 2.0 + 1.0)
...
#################################
def extract_spectra(det_list: List[InfrasoundDetection], st_list: List[Stream], win_buffer: float = 0.25,
                    ns_opt: str = "pre") -> np.ndarray:
    """
    ...
    for j in range(det_cnt):
        ...
        if ns_opt == "pre" or ns_opt == "post":
            ...
            X_sig, residual = beamforming_new.extract_signal(
                X_temp, f_sig, np.array([det_list[j].back_azimuth, det_list[j].trace_velocity]), geom)
...
def run(det_list: List[InfrasoundDetection], smn_spec: np.ndarray, src_loc: Tuple[float, float],
        freq_band: Tuple[float, float], tloss_models, resol: int = 150,
        yld_rng: Tuple[float, float] = np.array([10.0, 10.0e3]), ref_src_rng: float = 1.0, grnd_brst: bool = True,
        p_amb: float = 101.325, T_amb: float = 288.15, exp_type: str = "chemical") -> dict:
    """
    ...
    freqs = np.logspace(np.log10(max(tloss_models[0][0], freq_band[0])),
                        np.log10(min(tloss_models[0][-1], freq_band[1])), resol)
    ...
    psd_fit = interp2d(freqs, src_spec_vals + 10.0 * np.log10(1.0 / ref_src_rng),
                       np.product(pdf, axis=0).reshape((resol, resol)))
    ...
    for n in range(len(yld_vals)):
        yield_mult = 2.0 if grnd_brst else 1.0

        def temp(f):
            return psd_fit(f, 10.0 * np.log10(blastwave_spectrum(f, yld_vals[n] * yield_mult, ref_src_rng, p_amb, T_amb,
                                                                 exp_type))) / f
    ...
    result = {'spec_freqs': freqs, 'spec_vals': src_spec_vals + 10.0 * np.log10(1.0 / ref_src_rng),
              'spec_pdf': np.product(pdf, axis=0).reshape((resol, resol)), 'yld_vals': yld_vals / 1.0e3,
              'yld_pdf': yld_pdf, 'conf_bnds': np.array(conf_bnds) / 1.0e3}

    return result


def _single_station(det: InfrasoundDetection, src_spec, src_loc: Tuple[float, float], tlms,
                    freq_lims: Tuple[float, float], resolution: int, ref_rng: float):
    f_grid, spec_grid, pdf = det.src_spec_pdf(src_loc[0], src_loc[1],
                                              np.logspace(np.log10(max(freq_lims[0], tlms[0][0])),
                                                          np.log10(min(freq_lims[1], tlms[0][-1])), resolution),
                                              np.linspace(max(src_spec[1]) - 10.0, max(src_spec[1]) + 40.0, resolution),
                                              src_spec, tlms)
...
def _combine(file_list: List[str], yld_rng: Tuple[float, float], ref_rng: float, resolution: int, p_amb: float,
             T_amb: float, grnd_burst: bool, exp_type: str) -> dict:
    ...
    for file in file_list:
        ...
        with np.load(file) as data:
            ...
            PDF_interp = PDF_interp + [LinearNDInterpolator(list(zip(data['arr_0'], data['arr_1'])), data['arr_2'],
                                                            fill_value=0.0)]
    ...
    for n in range(len(yld_vals)):
        yield_mult = 2.0 if grnd_burst else 1.0

        def temp(f):
            return psd_fit(f, 10.0 * np.log10(blastwave_spectrum(f, yld_vals[n] * yield_mult, ref_rng, p_amb, T_amb,
                                                                 exp_type))) / f
