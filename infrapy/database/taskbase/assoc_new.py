...
class AssocInfraPy_LANL(Base):
    ...
    def __init__(self, conf_file=[]):
        ...
        if len(self.fdtables_names) == 0:
            print('NO tables with fd results were included, define one fd results tables, or include Fd_results '
                  '(this is the table where results are written by default, but needs to be specified ) ')
            sys.exit()
    ...
    def data_retrievingS(self, win_start, win_end):
        ...
        for aai in refstations_l:
            ...
            refsta.append({'lon': np.mean(array_lo), 'lat': np.mean(array_la), 'elev': np.mean(array_el), 'name': aai,
                           'numsta': len(array_la)})
        self.det_tot = []
        self.fdtable_name = []
        # embed()
        ...
        for aai in refsta:
            ...
            for ti in range(self.num_tables):
                try:
                    fd_res = self.session.query(self.fdtables[ti]).filter(self.fdtables[ti].sta == aai['name'])\
                        .filter(self.fdtables[ti].pfdid == self.pfdid).filter(self.fdtables[ti].pfkid == self.pfkid)\
                        .filter(self.fdtables[ti].timeini >= self.time_initial).all()
                    # times_ini=self.session.query(self.fdtables[ti].timeini).filter(self.fdtables[ti].sta==aai['name'])\
                    # .filter(self.fdtables[ti].pfdid==self.pfdid).filter(self.fdtables[ti].pfkid==self.pfkid)\
                    # .filter(self.fdtables[ti].timeini>=self.time_initial).filter(self.fdtables[ti]\
                    # .timeini<=self.time_end).all()
                    try:
                        class Fk_results(schema.fk_results):
                            __tablename__ = fd_res[0].fktablename
                    except Exception:
                        pass
                    times_ini = self.session.query(self.fdtables[ti].timeini)\
                        .filter(self.fdtables[ti].sta == aai['name']).filter(self.fdtables[ti].pfdid == self.pfdid)\
                        .filter(self.fdtables[ti].pfkid == self.pfkid)\
                        .filter(self.fdtables[ti].timeini >= self.time_initial).all()
                    if len(fd_res) > 0:
                        ...
                        for dqi in range(len(fd_res)):
                            if win_start < fd_res[dqi].timeini < win_end:
                                ...
                                fk_res = self.session.query(Fk_results).filter(Fk_results.pfkid == fd_res[dqi].pfkid)\
                                    .filter(Fk_results.sta == aai['name'])\
                                    .filter(Fk_results.timeini.between(fd_res[dqi].timeini, fd_res[dqi].timeend)).all()
                                embed()
                                fval = []
                                bz = []
                                ...
                                if not fvalm_i:
                                    ...
                                    # det1 = inf_det_global(aai['lat'], aai['lon'],
                                    # UTCDateTime(fd_res.timeini).datetime, fd_res.bz, fd_res.fval, aai['numsta'])
                                    det1 = (aai['lat'], aai['lon'], fd_res[dqi].timeini, bz[fvalm_i],  fval[fvalm_i],
                                            aai['numsta'], fd_res[dqi].fdid, aai['name'], self.fdtables_names[ti])
    ...
    def data_processingASSOC(self):
        ...
        if len(det_list) > 1:
            try:
                # EVIDs,DAQ,CAQ=assoc(det_list, self.lims, float(self.assocthresh), show_result=False, parallel=True,
                # num_cores=self.numcores)
                # labels, dists = hjl.run(det_list, self.clusterthresh, dist_max=self.distmax, bm_width=self.beamwidth,
                # rng_max=self.rangemax, trimming_thresh=self.trimthresh, pool=self.pl)
                labels, dists = hjl.run(det_list, self.clusterthresh, dist_max=self.distmax, bm_width=self.beamwidth,
                                        rng_max=self.rangemax,  pool=self.pl)
            ...
            for ii in range(len(EVIDsS)):
                EVIDsN.append({'ID':self.det_tot[sorted_index[ii]].ID,'eventID':EVIDsS[ii],'qdetcluster':DAQS[ii],
                'qassoc':CAQS[ii],'fdtable':self.fdtable_nameS[ii]})
            lastEVENTIDQ=self.session.query(func.max(self.ASSOC_results.eventid)).all()
            lastEVENTID=lastEVENTIDQ[0][0]
            if lastEVENTID is None:
                lastEVENTID=int(0)
