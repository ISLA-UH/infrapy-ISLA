...
class FDet(Base):
    ...
    classdocs
    '''
    def __init__(self, conf_file: str = "", ARRAY_NAME: Optional[str] = None):
        '''
        Constructor
        '''
    ...
    def data_retrieving(self, dayofyear):
        ...
        self.nchan = self.session.query(self.FK_results.nchan)\
            .filter(self.FK_results.timeini >= (t0 - self.prewindowlen))\
        ...
        self.fkdayres = self.session.query(self.FK_results)\
            .filter(self.FK_results.timeini >= (t0 - self.prewindowlen))\
            .filter(self.FK_results.timeini <= (te + self.prewindowlen))\
            .filter(self.FK_results.pfkid == self.pfkid)\
            .filter(self.FK_results.sta == self.sta).order_by(self.FK_results.timeini).all()
        ...
        self.time_lim = self.session.query(func.min(self.FK_results.timeini), func.max(self.FK_results.timeini))\
            .filter(self.FK_results.timeini > (t0 - self.prewindowlen))\
            .filter(self.FK_results.timeini < (te + self.prewindowlen))\
            .filter(self.FK_results.pfkid == self.pfkid)\
    ...
    def event_detection(self):
        ...
        detections = self.getdet(time_pf, fval_corr, pf, 1 - self.pthreshold, corr, self.cthr,
                                 int(self.minlen / int(self.fkpar.beamwinlen - self.fkpar.beamwinstep)),
                                 fval=fval_fk_effective, c=c_val)
        print(f'dofnum: {self.dofnum}, dofden: {self.dofden}')
        ...
        if len(detections) > 0:
            for idet in range(len(detections)):
                ...
                if not bool(id_res):
                    ...
                    self.session.add(res)
                    self.session.commit()
                    # print('det: ', short_time(UTCDateTime(detections[idet]['ti'])),' len(s):',
                    # float(detections[idet]['te'])-float(detections[idet]['ti']),' max fval:',detections[idet]['maxfo'])
                else:
    ...
    def event_detection2(self):
        ...
        times = tt
        M = self.nchan
        # detections = beamforming_new.detect_signals(times, beam_results, float(self.detwinlen),
        # det_thresh=float(self.pthresholdeshold), min_seq=int(self.dsegmin), method=self.detmethod,
        # TB_prod=self.tb_prod,channel_cnt=M)

        # detections = beamforming_new.detect_signals(times, beam_results, float(self.detwinlen),
        # det_thresh=float(1-self.pthreshold), min_seq=int(self.dsegmin), back_az_lim=self.backazlim,
        # method=self.detmethod, TB_prod=self.tb_prod,channel_cnt=M, use_det_mask=False)
        detections = beamforming_new.detect_signals(times, beam_results, float(self.adapwlen),
                                                    float(self.detwinlen) * band_w, M,
                                                    min_seq=int(self.dsegmin),
                                                    back_az_lim=self.backazlim, fixed_thresh=float(1-self.pthreshold))
        if len(detections) > 0:
            for idet in range(len(detections)):
                ...
                if not bool(id_res):
                    ...
                    self.session.commit()
                    # print('det: ', short_time(UTCDateTime(detections[idet]['ti'])),' len(s):',
                    # float(detections[idet]['te'])-float(detections[idet]['ti']),' max fval:',
                    # detections[idet]['maxfo'])
                else:
                    print('detection already in table')
        else:
            print('No detections in this day')
...
if __name__ == '__main__':
    pdetect = FDet('../conf_files/IMS_CONF_FILES/InfraConfigI57US')
