...
def stream2mult_array(str, wlen, overlap) -> Tuple[np.ndarray, List[UTCDateTime], list]:
...
def extARRAYPROC_ZIP(argu):
    ...
    So we have to call a method from within the class to run multicores. you provide a
     - ** parameters**, **types**, **return**, and **return types**::
         :param argu: list of lists with different elements
    ...
    try:
        global status
        try:
            if argu[14] not in ["bartlett", "music", "music32", "music16", "music32D", "music16D"]:
                print('the method requested for processing is not available')
                exit()
                # method='bartlett'
            # ret=beamforming.fkBARLET(argu[0],argu[1],argu[2],argu[3],argu[4],argu[5],argu[6],argu[8],argu[10],
            # argu[11],argu[12],argu[13])
            ret = beamforming.fkPROC(argu[14], argu[0], argu[1], argu[2], argu[3], argu[4], argu[5], argu[6],
                                     argu[8], argu[10], argu[11], argu[12], argu[13])
        except Exception as ex1:
...
class Fk(Base):
    ...
    def __init__(self, conf_file: str = "", ARRAY_NAME=None):
    ...
    def database_connecting(self):
        ...
        The system connects to the database and establishes the health of the connection, and writes the parameters
        for processing
         - ** parameters**, **types**, **return**, and **return types**::
        ...
        Update: Feb 2018
        DB connection established with a user-defined configuration file to specify which standard database tables a
        user is targeting.
        Pisces provides a function that reads the format and returns the necessary tables. The configuration file also
        specifies infrapy-specific database tables for fk processing
        """
    ...
    def data_retrieving(self, dayofyear: int) -> int:
        """ retrieve one day of data from the database and write back results
         ...
         - ** parameters**, **types**, **return**, and **return types**::
            ...
            :return: integer with state of health of the stream, 0 problem with retrieving the data;
                    1 this data was already analyzed; 2 successful retrieving the data stream
            :rtype: integer
        ...
        try:
            self.aa = get_arraywaveforms(self.session, self.Site, self.Wfdisc, self.sta, t0=t0, te=te,
                                         channel=self.chan)
            if len(self.aa[0]) < 60:
                return 0
        except Exception as ex1:
    ...
    def array_processing(self):
        ...
        try:
            ...
            for aai in self.aa:
                # if (type(aai.data) is np.ma.core.MaskedArray):
        ...
        slow = self.slow
        self.fN, self.freqsN, self.mult_vectors = beamforming._mult_vectors(win_l, sps, slow, x, y, nchan,
                                                                            fmin=self.freqmin, fmax=self.freqmax)
        ...
        for tre_i in range(len(arraySTREAM)):
            argumentSTREAMS.append([arraySTREAM[tre_i], sps, self.slow, self.mult_vectors, self.fN, X, Y,
                                    len(arraySTREAM), arraySTREAM_time[tre_i], self.cpucnt, self.func_fk,
                                    self.freqsN, self.sta, self.numsources, self.algorithm])
        if self.cpucnt > 1:
        ...
        for r_T in range(len(out)):
            for r_i in range(len(out[r_T])):
                try:
                    res = self.FK_results(pfkid=self.FK_parQuery.pfkid, timeini=float(arraySTREAM_time[r_T]),
                                          timeend=float(arraySTREAM_time[r_T] + window_l), fkid=id_resC, sta=self.sta,
                                          bz=out[r_T][r_i][0], slofk=out[r_T][r_i][1], fval=out[r_T][r_i][2],
                                          xcorrvalmin=out[r_T][r_i][3], xcorrvalmax=out[r_T][r_i][4],
    ...
    def array_processing_2(self):
        ...
        for win_start in np.arange(t[0], t[-1], self.beamwinstep):
            ...
            beam_power = beamforming_new.run(X, S, f, geom, delays, [self.freqmin, self.freqmax], method=self.algorithm,
                                             pool=pl, normalize_beam=True)
            peaks = beamforming_new.find_peaks(beam_power, back_az_vals, trc_vel_vals, signal_cnt=1)
            beam_results = beam_results + [[peaks[0][0], peaks[0][1], peaks[0][2], peaks[0][2] / (1.0 - peaks[0][2])
                                            * (x.shape[0] - 1)]]
            # embed()
...
if __name__ == '__main__':
    ...
    print('run FK, with configuration param:', sys.argv[1])
    pdetect = Fk(sys.argv[1], ARRAY_NAME=AR)
