...
user_config.read("sandbox\\automated_detection\\config")
...
nrt_stime = obspy.UTCDateTime("2025-10-11T18:30:00.000000Z")
    ...
    while i < 9999999:
        try:
            ...
            EVENT_CONFIG = {
                ...
                "channel": "BDF",
                "start_time": (
                    obspy.UTCDateTime() - 720 if (real_time) else nrt_stime + (i * 480)
                ),
                "end_time": (
                    obspy.UTCDateTime() - 120 if (real_time) else nrt_stime + (i * 480) + 600
                ),
            }
            ...
            # Save detections
            if len(det_list) > 0:
                det_fpath = "sandbox\\automated_detection\\results\\" + t1.strftime("%Y/%m/%d/")
                try:
                    ...
                    det_fpath = "sandbox\\automated_detection\\results\\bin\\"
