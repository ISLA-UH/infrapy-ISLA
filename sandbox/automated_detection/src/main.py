...
if __name__ == "__main__":
    ...
    cfg_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(root_path, 'config', 'example.ini')
    # Setup Event Detector and paths
    evd = EventDetector.load_config(cfg_path)

    results_path = evd.results_dir if evd.results_dir else os.path.join(root_path, 'results')
    ...
    trace_vel_min: float = infraconfig.get_param(evd.user_config, "FK", "trace_vel_min", None, "float")  # type: ignore
    trace_vel_max: float = infraconfig.get_param(evd.user_config, "FK", "trace_vel_max", None, "float")  # type: ignore
    ...
    sub_window_len: float = infraconfig.get_param(evd.user_config, "FK", "sub_window_len", None,
                                                  "float")  # type: ignore
    ...
    back_az_width: float = infraconfig.get_param(evd.user_config, "FD", "back_az_width", None, "float")  # type: ignore
