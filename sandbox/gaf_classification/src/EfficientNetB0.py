...
if __name__ == "__main__":
    ...
    # Model needs int labels, convert from string
    Y_train = np.array([])
    ...
    model, base_model = build_efficientnetb0(input_shape=X_train.shape[1:], num_classes=2)
    model.compile(
    ...
    print("\n=== CLASS BALANCE ===")
    print(f"Training set: {dict(zip(['SonicBoom(0)', 'Surf(1)'], counts))}")
    print(f"Class ratio (0:1): {counts[0]/counts[1]:.2f}")

    unique_val, counts_val = np.unique(Y_val, return_counts=True)
    print(f"Validation set: {dict(zip(['SonicBoom(0)', 'Surf(1)'], counts_val))}")

    unique_test, counts_test = np.unique(Y_test, return_counts=True)
    print(f"Test set: {dict(zip(['SonicBoom(0)', 'Surf(1)'], counts_test))}")
    print("===================\n")
    print(f"Input min/max: {X_train.min():.4f} / {X_train.max():.4f}")

    total_samples = len(Y_train)
    n_class_0 = np.sum(Y_train == 0)  # SonicBoom count
    n_class_1 = np.sum(Y_train == 1)  # Surf count
    weight_0 = total_samples / (2.0 * n_class_0) if n_class_0 > 0 else 1.0
    weight_1 = total_samples / (2.0 * n_class_1) if n_class_1 > 0 else 1.0
    
    weights_dict = {
        0: (weight_0),  # Weight for SonicBoom
        1: (weight_1*1.4)   # Weight for Surf
    }
    ...
    model.load_weights('best_efficientnet.h5') 
    ...
    y_pred_class = (preds.flatten() > 0.4).astype(int)
    y_true_class = Y_test.astype(int).flatten()
    pred_unique, pred_counts = np.unique(y_pred_class, return_counts=True)
    print(f"Prediction distribution: {dict(zip(pred_unique, pred_counts))}")
